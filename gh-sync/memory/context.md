# Context — Current System State

> Updated after each wave. Read at wave start.

## Last Updated
2026-06-20 02:55 UTC+8

## System Status
- **Phase**: Post-Compliance Evolution (Wave 60)
- **Waves in DB**: 54
- **Spec compliance**: 100% (15/15, dynamic check)

## Current Metrics
| Metric | Value |
|--------|-------|
| API routes | 18 |
| Dashboard tabs | 6 |
| Skills | 14 |
| GitHub commits | 104 |
| Waves in DB | 54 |
| Wave success rate (recent 5) | 100% |
| Health score | ~90/100 (stable) |
| Spanish strings in src/ | 0 |

## What exists
- Dashboard: single-fetch, 3-col overview, Decision Timeline, wave velocity, duration column
- Overview: health score badge **with trend arrow** (▲▼●), responsive sparklines, error banner with retry
- Agent Live: 3D avatar, phase tracker, health bar **with trend arrow**, error state indicators, standby with countdown
- All 6 tabs: explicit error state handling
- **Keyboard shortcut help**: `?` button in footer with animated popover
- Hooks: API_BASE constant, healthScore + healthScoreTrend synced to agent-live-store
- Dashboard API: returns healthScoreTrend derived from error trend + success rate
- UX: Keyboard shortcuts (1-6), kbd badges on lg+, ? help on all sizes
- Footer: "Agent = Model + Harness" + "Spec-Driven Self-Evolution" + ? button
- Package: @hermes/harness-dashboard v0.1.0

## What's next
1. Watch for skills route disappearing again
2. Explore adding a wave comparison or diff view
3. Consider outcome tracking for decisions (outcome field is always null)